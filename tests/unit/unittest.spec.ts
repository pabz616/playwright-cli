...
test.describe('Demoblaze Product Store Test Data Config', () => {
    ...
    test("Confirms valid categories", () => {
        ...
        expect(getCategorySelector("Laptops")).toContain(testData.PRODUCT_CATEGORY2);
        expect(getCategorySelector("Monitors")).toContain(testData.PRODUCT_CATEGORY3);
    });
    ...
    test('Confirm home page navigation and main site links', () => {
        expect(testData.NAV_LINKS[0]).toBe('text=Home');
        ...
        expect(testData.NAV_LINKS[5]).toBe('#signin2');
     });
